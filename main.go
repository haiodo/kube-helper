// Copyright (c) 2020 Andrey Sobolev and/or its affiliates.
//
...
`)
	...
	if args[0] == "port-forward" && len(args) == 5 {
		...
		for {
			pods, err := ListPods(namespace, pattern, map[string]string{})
			...
			for {
				pods2, err := ListPods(namespace, pattern, map[string]string{})
				if err != nil {
					logrus.Fatalf("failed to find pods %v", err)
				}
	...
	if args[0] == "logs" && len(args) == 3 {
		for {
			...
			pods, err := ListPods(args[1], args[2], map[string]string{})
			if err != nil {
